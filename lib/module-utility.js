...
function createModule() {
	...
	inquire(questions()).then(function (answers) {
		var moduleType = answers.moduleType,
			...
			files = answers.files,
			path = resolveModuleDir(moduleType, moduleName);
		//check if the module already exists
		return getModuleStatus(path).then(function (status) {
			//the module already exists, create files that don't exist yet
			if(status){
				return getMissingFileTypes(files, status.files);
			}
			//the module does not exist yet: create a directory for it
			createModuleDir(path);
		});
	});
}
function editModule() {
