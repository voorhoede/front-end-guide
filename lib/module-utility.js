...
function editModule() {
	...
	return inquire(editQuestions).then(function (answers) {
		var files = answers.files,
			...
			module = {};
		if(!files){
			throw new Error('not creating any files');
		}
...
function removeModule() {
	...
	return inquire(removeQuestions).then(function (answers) {
		...
		//user has confirmed the wish to delete the designated module
		var dir = resolveModuleDir.apply(null, answers.path.split('/'));
